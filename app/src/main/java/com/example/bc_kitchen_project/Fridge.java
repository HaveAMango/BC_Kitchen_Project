...
public class Fridge extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        database = FirebaseDatabase.getInstance().getReference("user-fridge").child(activeUserId);
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                switch (which) {
                                    case DialogInterface.BUTTON_POSITIVE:
                                        database = FirebaseDatabase.getInstance().getReference("user-fridge").child(activeUserId).child(product);
                                        database.addListenerForSingleValueEvent(new ValueEventListener() {
                                            @Override
                                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                                int countCurrent = Integer.parseInt(snapshot.child("count").getValue().toString());
                                                if (countCurrent == 1) {
                                                    DatabaseReference temp = FirebaseDatabase.getInstance().getReference("user-fridge").child(activeUserId).child(product);
